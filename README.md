...
- **Unified & Grouped Dashboard**: 
  ...
  - **Deep-Linking**: Supports direct anchors for individual image groups and versions. The URL automatically updates when an item is expanded or viewed, allowing users to share links that automatically expand, scroll to, and highlight the targeted artifact on load.
...
- **cosign**: Must be installed and available in your PATH.
  - Installation on macOS: `brew install cosign`
  - Installation on Linux: `go install github.com/sigstore/cosign/v2/cmd/cosign@latest`
...
Inspects images directly from the `registry.suse.com/ai/` namespace and extracts any embedded SBOMs.
```bash
...
- `fetch_suse_registry_images.py`: Inspector for the SUSE AI Registry and SBOM extractor.
